...
public abstract class BaseHouse implements House {
    ...
    public int sumSoftness() {
        int sumSoftnessOfToys = 0;
        for (Toy toy : toys) {
            sumSoftnessOfToys += toy.getSoftness();
        }

        return sumSoftnessOfToys;
    }

    @Override
    public void addCat(Cat cat) {
        if (capacity <= this.getCats().size()) {
    ...
    public String getStatistics() {
        ...
        sb.append(String.format("Toys: %d Softness: %d", this.getToys().size(), sumSoftness())).append(System.lineSeparator());
